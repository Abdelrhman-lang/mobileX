...
export default function HeaderActions() {
  ...
  if (loading) return <Spinner className="w-3 h-3" />
  return (
    <div className="">
          ...
          {isSignedIn ? (
            <UserButton>
              <UserButton.MenuItems>
                {userData?.role === "super admin" || userData?.role === "admin" ? <UserButton.Link label="Go To Dashboard" href="/dash" labelIcon={<Package className="w-4 h-4" />} /> : null}
                <UserButton.Link label="View Your Account" href="/user-account" labelIcon={<User className="w-4 h-4" />} />
              </UserButton.MenuItems>
            </UserButton>
          ) : (
